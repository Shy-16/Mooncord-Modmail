...
async def create_ticket(bot: discord.Client, ctx: discord.Context, params: list) -> None:
	# 'attachments', 'author', 'channel_id', 'components', 'content', 'edited_timestamp', 'embeds', 'flags',
	# 'guild_id', 'id', 'member', 'mention_everyone', 'mention_roles', 'mentions', 'nonce', 'pinned', 'referenced_ctx', 'timestamp', 'tts', 'type'
	...
	if not ticket:
		await bot.http.send_ctx(ctx.channel_id, 
	...
	await bot.send_embed_dm(ctx.author.id, "Ticket closed", f"Ticket {ticket['_id']} was closed.", color=10038562, fields=fields, footer=footer)
	await bot.send_embed_ctx(bot.guild_config[ctx.guild_id]['modmail_channel'], "Ticket closed", f"Ticket {ticket['_id']} was closed.", color=10038562, fields=fields, footer=footer)
